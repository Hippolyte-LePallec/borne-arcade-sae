...
public class Serpent {
	...
	public Serpent(Fenetre f, Carre c) {
		...
		serpent.add(c);
		tete = c;
		Carre corps_1 = new Carre(Couleur.BLANC, new Point(tete.getA().getX() - taillecarre, tete.getA().getY()),
				taillecarre, true);
		serpent.add(corps_1);
		Carre corps_2 = new Carre(Couleur.BLANC, new Point(tete.getA().getX() - taillecarre, tete.getA().getY()),
				taillecarre, true);
		serpent.add(corps_2);
	...
	public void intersection(ArrayList<Pomme> a) {
		...
		for (int i = 1; i < serpent.size(); i++) {
			...
			if ((serpent.get(i).getA().getX() < tete.getB().getX() && serpent.get(i).getA().getY() < tete.getB().getY())
					&&
					(serpent.get(i).getB().getX() > tete.getA().getX()
							&& serpent.get(i).getB().getY() > tete.getA().getY()))
				jouer = false;
		}
		...
		for (int i = 0; i < a.size(); i++) {
			...
			if ((a.get(i).getC().getA().getX() < tete.getB().getX()
					&& a.get(i).getC().getA().getY() < tete.getB().getY()) &&
					(a.get(i).getC().getB().getX() > tete.getA().getX()
							&& a.get(i).getC().getB().getY() > tete.getA().getY())) {
	...
	public void mouvement(ClavierBorneArcade c) {
		...
		for (int i = serpent.size() - 1; i > 0; i--) {
			serpent.get(i).translater(serpent.get(i - 1).getA().getX() - serpent.get(i).getA().getX(),
					serpent.get(i - 1).getA().getY() - serpent.get(i).getA().getY());
		}
